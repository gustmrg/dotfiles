...
`bootstrap.sh` links `shell/.zshrc` to `~/.zshrc` by default or copies it with `--copy`. Keep machine-specific secrets, host-only paths, and private tokens out of this file; put them in the unversioned `~/.zshrc.local`, which this file sources last when present.
